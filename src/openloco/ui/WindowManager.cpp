...
namespace openloco::ui::WindowManager
{
    ...
    {
        auto it = std::find_if(_windows.begin(), _windows.end(), [&w](std::unique_ptr<ui::window> const& window) {
            return window.get() == w;
        });
    ...
    void copy_rect(int16_t ax, int16_t bx, int16_t cx, int16_t dx, int16_t di, int16_t si)
    {
    ...
    void sub_4C6B09(window* window, viewport* viewport, int16_t x, int16_t y)
    {
        if (window != nullptr)
        {
            ...
            {
                auto nextWindowIndex = WindowManager::indexOf(window) + 1;
