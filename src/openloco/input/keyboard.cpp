...
#pragma pack(pop)
    ...
    static bool try_shortcut(shortcuts::Shortcut sc, uint32_t keyCode, uint8_t modifiers)
    {
        auto cfg = openloco::config::get();
        if (cfg.keyboard_shortcuts[sc].var_0 == keyCode && cfg.keyboard_shortcuts[sc].var_1 == modifiers)
        {
            shortcuts::execute(sc);
            return true;
        }
    ...
    static void normal_key()
    {
        while (true)
        {
            ...
            if (ti != nullptr)
            {
                if (try_shortcut(shortcuts::Shortcut::screenshot, eax->keyCode, _keyModifier))
                    continue;
            ...
            if (!is_title_mode())
            {
                for (int i = 0; i < 35; i++)
                {
                    if (try_shortcut((shortcuts::Shortcut)i, eax->keyCode, _keyModifier))
                        break;
                }
                continue;
            }
            ...
            if (try_shortcut(shortcuts::Shortcut::Send_Message, eax->keyCode, _keyModifier))
                continue;
